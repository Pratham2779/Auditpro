...
import { Bell, Send, Inbox, MessageCircle, User, ChevronLeft, ChevronRight, Search, Users } from 'lucide-react';
...
function AuditorNotifications() {

  const location=useLocation();
  const {quickAction} = location.state || {};


  const [activeTab, setActiveTab] = useState(quickAction?quickAction:'compose');
  ...
  const [admins, setAdmins] = useState([]); // Now storing multiple admins

  useEffect(() => {
    fetchAdminInfo(); // Fetch admin info when component mounts
    fetchNotifications();
  }, [dispatch]);
  ...
  const fetchAdminInfo = async () => {
    try {
      const adminsList = await getAllUsers({ role: 'admin' });
  ...
  const handleReceiverChange = (e) => {
    ...
    setComposeData(prev => ({ ...prev, receivers: selectedOptions }));
  };

  const handleSendNotification = async () => {
    if (!composeData.content || composeData.receivers.length === 0) return;

    try {
      // Send notification to each selected admin
      await Promise.all(
        composeData.receivers.map(adminId =>
          sendNotification({
            receiver: adminId,
      ...
      // Reset form
      setComposeData({ subject: '', content: '', receivers: [] });
    } catch (err) {
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-center space-x-3">
            ...
            <div className="text-center">
              ...
              <p className="text-gray-600 mt-1">Communicate with system administrators</p>
            </div>
          </div>
        </div>
      </div>
                ...
                <div className="p-6 space-y-6">
                  ...
                  <div className="space-y-2 relative">
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      {composeData.receivers.length} admin(s) selected
                    </p>
                  </div>
                  ...
                  <div className="pt-4">
                    <button
                      ...
                      disabled={!composeData.content || composeData.receivers.length === 0}
                ...
                {loading ? (
                  ...
                  <div className="divide-y divide-gray-100">
                    {paginatedNotifications.map((notification) => {
                      const isSent = notification.sender._id === user._id;
                      const contact = isSent ? notification.receiver : notification.sender;

                      return (
                        <div
                          key={notification._id}
                          className={`p-4 hover:bg-gray-50 transition-colors relative group ${notification.status === 'unread' && !isSent
                            ...
                            <div className="flex items-center space-x-3">
                              ...
                              <div>
                                <div className="flex items-center space-x-2">
                                  <span className="font-medium text-gray-800">{contact.fullName}</span>
                                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${isSent ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'
                                    }`}>
